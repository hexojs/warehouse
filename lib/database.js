...
Database.prototype.load = function(callback){
  ...
  return new Promise(function(resolve, reject){
      ...
      switch (keys.shift()){
        case 'meta':
          if (keys.shift() === 'version'){
            upgrade = value - self.options.version;
          }
    ...
    if (upgrade > 0){
      return Promise.method(self.options.onUpgrade);
    } else if (upgrade < 0){
      return Promise.method(self.options.onDowngrade);
    }
  }).nodeify(callback);
};
...
Database.prototype.save = function(callback){
  ...
  return new Promise(function(resolve, reject){
    ...
    for (var i = 0, len = keys.length; i < len; i++){
      ...
      if (i) stream.write(',');
      if (model) stream.write('"' + key + '":' + model._export());
    }

...
Model.prototype.reverse = function(){
  var index = reverse(Object.keys(this.data)),
    len = this.length,
    arr = new Array(len);
...
Model.prototype.shuffle = function(){
  var index = shuffle(Object.keys(this.data)),
    len = this.length,
    arr = new Array(len);
...
Model.prototype.populate = function(expr){
  var keys = Object.keys(this.data),
    stack = this.schema._parsePopulate(expr),
    arr = [];

  for (var i = 0, len = this.length; i < len; i++){
    arr.push(this._populate(this.findById(keys[i]), stack));
  }

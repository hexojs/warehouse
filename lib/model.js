...
var Model = module.exports = function(name, parent, schema, index, fields){
  this._name = name;
  this._parent = parent;
  this._store = parent.store[name];
  this._index = index;
  this._fields = fields;
  this._schema = schema;
...
Model.prototype.get = function(){
  ...
  if (args.length > 1){
    var id = _(args).flatten().uniq(),
      arr = [];
    for (var i=0, len=id.length; i<len; i++){
      var data = this._getOne(id[i]);
      if (typeof data !== 'undefined') arr.push(data);
    }
    return arr;
  } else {
    var id = args[0];
...
Model.prototype.insert = function(data, callback){
  ...
  if (_.isArray(data)){
    ...
    for (var i=0, len=obj.length; i<len; i++){
      this._insertOne(obj[i], function(item){
        arr.push(item);
      });
    }
...
Model.prototype.remove = function(id){
  if (typeof id === 'undefined'){
    ...
    for (var i=0, len=this.length; i<len; i++){
      this._store.remove(index[i]);
    }
  } else if (_.isArray(id)){
    for (var i=0, len=id.length; i<len; i++){
      this._store.remove(id[i]);
    }
  } else {

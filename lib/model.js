...
Model.prototype._insertOne = function(data_) {
  ...
  // Apply getters
  const data = data_ instanceof self.Document ? data_ : self.new(data_);
  const id = data._id;
...
Model.prototype.insertOne = function(data, callback) {
  ...
  return Promise.using(this._acquireWriteLock(), () => self._insertOne(data)).asCallback(callback);
};
...
Model.prototype.insert = function(data, callback) {
  if (isArray(data)) {
    ...
    return Promise.mapSeries(data, item => self.insertOne(item)).asCallback(callback);
  }
...
Model.prototype.updateById = function(id, update, callback) {
  ...
  return Promise.using(this._acquireWriteLock(), () => {
    const stack = self.schema._parseUpdate(update);
...
Model.prototype.replaceById = function(id, data, callback) {
  ...
  return Promise.using(this._acquireWriteLock(), () => self._replaceById(id, data)).asCallback(callback);
};
...
Model.prototype.removeById = function(id, callback) {
  ...
  return Promise.using(this._acquireWriteLock(), () => self._removeById(id)).asCallback(callback);
};

...
Model.prototype.get = Model.prototype.findById = function(id){
  ...
  //var doc = new this._doc(this._store[id]),
  //var data = _.clone(this._store[id]),
    /*schema = this.schema,
    paths = schema.paths,
  ...
  if (path.type === Virtual){
    ...
    if (_.isObject(castValue)){
      ...
      for (var i = 0, len = keys.length; i < len; i++){
        ...
        var operator = path.updateOperators[key.slice(1)];
        if (!operator) continue;

        castValue = operator(obj[key], castValue[key]);
      }
    }
...
Model.prototype.updateById = function(id, obj, callback){
  ...
  this._save(_.extend(this._getRaw(id), obj), function(result){

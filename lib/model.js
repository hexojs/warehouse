...
function Model(name, schema_) {
   ...
   * @private
   */
  var lock = this._lock = new ReadWriteLock();
  lock.readLockAsync = Promise.promisify(lock.async.readLock);
  lock.writeLockAsync = Promise.promisify(lock.async.writeLock);
...
Model.prototype.insertOne = function(data_, callback) {
  ...
  return this._lock.writeLockAsync().then(function(release) {

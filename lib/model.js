...
Model.prototype._insertOne = function(data_) {
  ...
  // Check ID
  if (!id) {
    return Promise.reject(new WarehouseError('ID is not defined'));
  }

  if (this.data[id]) {
    return Promise.reject(new WarehouseError('ID `' + id + '` has been used'));
  }
...
Model.prototype.insertOne = function(data, callback) {
  return this._insertOne(data).asCallback(callback);
};
...
Model.prototype._updateWithStack = function(id, stack) {
  ...
  if (!data) {
    return Promise.reject(new WarehouseError('ID `' + id + '` does not exist'));
  }

  return Promise.using(this._acquireWriteLock(id), function() {
    // Clone data
...
Model.prototype.updateById = function(id, update, callback) {
  ...
  return this._updateWithStack(id, stack).asCallback(callback);
};
...
Model.prototype.replaceById = function(id, data, callback) {
  return this._replaceById(id, data).asCallback(callback);
};
...
Model.prototype.removeById = function(id, callback) {
  return this._removeById(id).asCallback(callback);
};

...
Model.prototype.get = function(){
  var args = _.uniq(_.flatten(_.toArray(arguments)));
...
Model.prototype._updateOne = function(id, obj){
  ...
  for (var i in obj){
    var target = obj[i],
      origin = data[i];
    if (data.hasOwnProperty(i) && _.isObject(target) && !_.isArray(target)){
      if (_.isArray(origin)){
        for (var j in target){
          var prop = target[j];
          switch (j){
...
// http://jsperf.com/javascript-array-copy/2
var _arrCopy = function(arr){
  return arr.slice();
};

// http://jsperf.com/js-array-reverse-vs-while-loop/8
...
Model.prototype.reverse = function(){
  return this._init(_arrReverse(this._index));
};
...
// http://jsperf.com/array-shuffle-comparator/3
var _arrRandom = function(arr){
  arr = _arrCopy(arr);
...
Model.prototype.random = Model.prototype.shuffle = function(){
  return this._init(_arrRandom(this._index));
};
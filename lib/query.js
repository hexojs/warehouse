...
Query.prototype.find = function(query, options_){
  var options = options_ || {},
    filter = this._schema._execQuery(query),
    data = this.data,
    i = 0,
    len = this.length,
  ...
  if (options.skip) arr = arr.slice(options.skip);
...
Query.prototype.sort = function(orderby, order){
  var sort = parseArgs(orderby, order),
    fn = this._schema._execSort(sort);
...
Query.prototype.update = function(data, callback){
  var model = this._model,
    stack = this._schema._parseUpdate(data);

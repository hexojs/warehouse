...
Query.prototype._createQuery = function(index){
  ...
  return new this._model._query(index);
};
...
var _query = function(data, conditions){
  var match = true,
...
Query.prototype.find = function(conditions, options){
  ...
  for (var i = 0, len = this.length; i < len; i++){
    if (_query(model._getRaw(index[i]), conditions)) arr.push(index[i]);

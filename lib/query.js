...
Query.prototype.sort = function(orderby, order){
  ...
  if (order){
    conditions.push([orderby, order]);
  } else {
    for (var i in orderby){
      conditions.push([i, orderby[i]]);
    }
  }
  ...
  var arr = this.toArray().sort(function(a, b){
    var item, _orderby, _order, orderA, orderB, result;

    for (var i = 0; i < length; i++){
      item = conditions[i];
      _orderby = item[0];
      _order = item[1];
      orderA = getProperty(a, _orderby);
      orderB = getProperty(b, _orderby);
      ...
      if (_order == -1 || _order === 'desc' || _order === 'descending'){
        return ~result + 1;
      } else {
        return result;
      }
  ...
  return this._createQuery(_.pluck(arr, '_id'));
};

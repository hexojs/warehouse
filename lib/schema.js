...
const updateStackNormal = (key, update) => data => {
  setProp(data, key, update);
};

const updateStackOperator = (path_, ukey, key, update) => {
...
const queryStackNormal = (path_, key, query) => {
  const path = path_ || new SchemaType(key);

  return data => path.match(getProp(data, key), query, data);
};

const queryStackOperator = (path_, qkey, key, query) => {
  const path = path_ || new SchemaType(key);

  return data => path[qkey](getProp(data, key), query, data);
};

const execQueryStack = stack => {
...
const $where = fn => data => fn.call(data);
...
class QueryParser {
  ...
  parseUpdate(updates, prefix = '') {
    ...
    for (let i = 0, len = keys.length; i < len; i++) {
      ...
      if (key[0] === '$') {
        ...
        stack = stack.concat(this.parseUpdate(update, `${name}.`));
      } else {
  ...
  parseNormalQuery(queries, prefix = '') {
    ...
    for (let i = 0, len = keys.length; i < len; i++) {
      ...
      if (key[0] === '$') {
        ...
        stack = stack.concat(this.parseNormalQuery(query, `${name}.`));
      } else {
  ...
  parseQuery(queries) {
    ...
    for (let i = 0, len = keys.length; i < len; i++) {
      ...
      switch (key) {
        case '$and':
          stack = stack.concat(this.parseQueryArray(query));
          break;

        case '$or':
          stack.push($or(this.parseQueryArray(query)));
          break;

        case '$nor':
          stack.push($nor(this.parseQueryArray(query)));
          break;

        case '$not':
          stack.push($not(this.parseQuery(query)));
          break;
        ...
        default:
          if (isPlainObject(query)) {
            stack = stack.concat(this.parseNormalQuery(query, `${key}.`));
          } else {
  ...
  execQuery(query) {
    return execQueryStack(this.parseQuery(query));
  }
}

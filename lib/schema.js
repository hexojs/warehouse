...
Schema.prototype._parseQuery = function(queries, prefix_){
  ...
  for (var i = 0, len = keys.length; i < len; i++){
    ...
    switch (key){
      ...
      case '$not':
        stack.push($not(this._parseQuery(query, prefix)));
        break;
  ...
  this.stacks.query[stackKey] = stack;
  return stack;
};
...
function sortStack(path_, key, sort){
  ...
  return function(a, b){
    var result = path.compare(util.getProp(a, key), util.getProp(b, key));
    return descending && result ? result * -1 : result;
  };
}

Schema.prototype._parseSort = function(sorts, prefix_){
  var stackKey = JSON.stringify(sorts),
    stack = this.stacks.sort[stackKey];
  ...
  this.stacks.sort[stackKey] = stack;
  return stack;
};
